...
<p>The basic structure of this project is a driver file is launched which calls the method build_data which builds the vocabulary for the various machine learning models and extract_features which builds a feature matrix and corresponding classification vector. Both the build_data and extract_features methods are in a python3 file called parse_data. The part of the project which extracts tweets is handled in the get_tweets python file. The control flow of the project is the driver is launched -> the vocabulary is built -> the features are extracted -> the training accuracy is predicted -> the testing features are extracted -> the testing vector is predicted -> then compared for accuracy -> user can input twitter handle and parse tweets from that user and run them through our model
  The three models offered are adaboosting decision trees, regulary decision trees, and naive bayes. The have accurate approximations in ascending order of how they were listed..</p>
<h2>Sprints</h2>
<h4>Sprint 1</h4>
